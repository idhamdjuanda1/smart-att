...
import {
  Activity, Ban, CheckCircle2, Clock3, FileText, KeyRound, LayoutDashboard, Loader2,
...
export function SuperAdminProfessional({ user, onLogout }: { user: User; onLogout: () => void }) {
  const [tab, setTab] = useState<"dashboard" | "users" | "tokens" | "monitor" | "articles">("dashboard");
...
export function ProfileProfessional({ user, demo, students, setToast }: { user: User | null; demo: boolean; students: ProfileStudent[]; setToast: (toast: Toast) => void }) {
  ...
  async function saveProfile() {
    ...
    setSaving(true);
    try { if (!demo && user) { const nextSchool = school.trim(); await updateDoc(doc(db, "users", user.uid), { name: name.trim(), schoolName: nextSchool, phone: phone.trim(), updatedAt: serverTimestamp() }); await setDoc(doc(db, "users", user.uid, "settings", "academic"), { schoolName: nextSchool, updatedAt: serverTimestamp() }, { merge: true }); } setToast({ message: "Profil guru berhasil diperbarui.", tone: "success" }); }
  ...
  return <>
    ...
    <div className="grid gap-6 xl:grid-cols-[1.2fr_.8fr]">
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
        <div className="grid gap-4 sm:grid-cols-2"><Input label="Nama guru" value={name} onChange={setName}/><Input label="Email login" value={demo ? "tolimur@gmail.com" : user?.email ?? ""} nonaktif/><Input label="Nomor WhatsApp" value={phone} onChange={setPhone}/><Input label="Sekolah utama" value={school} onChange={setSchool}/></div>
...
function AccountCard({ account, now, busy, onToggle }: { account: Account; now: number; busy: boolean; onToggle: () => void }) { return <article className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><div className="flex items-start justify-between gap-3"><div><h3 className="font-black">{account.name || "Belum diisi"}</h3><p className="text-xs text-slate-400">{account.email}</p></div><AccountBadge status={accountStatus(account, now)}/></div><div className="mt-3 grid grid-cols-2 gap-2"><MiniStat label="Sekolah" value={account.schoolName || "—"}/><MiniStat label="Kedaluwarsa" value={accountExpiry(account) ? new Date(accountExpiry(account)).toLocaleDateString("id-ID") : "—"}/><MiniStat label="Token" value={account.activeTokenId || "Belum ada"}/><MiniStat label="Login" value={dateTime(account.lastLoginAtMs)}/></div><button disabled={busy} onClick={onToggle} className={`mt-3 w-full rounded-xl py-2.5 text-xs font-black ${account.disabled ? "bg-emerald-50 text-emerald-700" : "bg-rose-50 text-rose-700"}`}>{account.disabled ? "Aktifkan akun" : "Nonaktif akun"}</button></article>; }
function Input({ label, value, onChange, nonaktif = false }: { label: string; value: string; onChange?: (value: string) => void; nonaktif?: boolean }) { return <label><span className="mb-2 block text-xs font-extrabold">{label}</span><input value={value} disabled={nonaktif} onChange={(event) => onChange?.(event.target.value)} className="h-12 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm outline-none focus:border-teal-500 nonaktif:bg-slate-50 nonaktif:text-slate-400"/></label>; }
