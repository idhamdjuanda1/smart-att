...
export function PublicArticles({slug}:{slug?:string}){
  ...
  const cleanSlug=slug?decodeURIComponent(slug).trim().toLowerCase().replace(/\/+$|^\/+/g,""):undefined;
  const article=cleanSlug?articles.find((item)=>item.slug.toLowerCase()===cleanSlug||item.id.toLowerCase()===cleanSlug):undefined;
