  ...
  return <main className="min-h-screen bg-[radial-gradient(circle_at_top_left,#ccfbf1,transparent_35%),linear-gradient(135deg,#f8fafc,#fff7ed)] px-4 py-8 sm:py-14">
    ...
    <section className="mx-auto max-w-lg rounded-[2rem] border border-white/80 bg-white p-7 shadow-2xl shadow-slate-900/10 sm:p-10">
        ...
        <form onSubmit={submit} className="mt-7">
          <label className="block"><span className="mb-2 block text-xs font-black text-slate-700">Kode ulangan (4 digit angka)</span><input autoFocus inputMode="numeric" pattern="[0-9]*" autoCapitalize="characters" autoComplete="off" spellCheck={false} maxLength={300} value={value} onChange={(event)=>{setValue(event.target.value);setError("")}} placeholder="Contoh: 1234" className="h-16 w-full rounded-2xl border-2 border-slate-200 px-5 text-center text-2xl font-black uppercase tracking-[.25em] text-slate-950 outline-none transition placeholder:text-sm placeholder:font-bold placeholder:normal-case placeholder:tracking-normal focus:border-teal-500 focus:ring-4 focus:ring-teal-500/10"/></label>
